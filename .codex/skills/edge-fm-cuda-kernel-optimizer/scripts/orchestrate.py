...
Typical invocation (Codex drives):
  ...
  # (Codex reads profile, writes iterv1/kernel.{cu|py} and methods.json)

  # step 3d-3f: validate + record
  python orchestrate.py close-iter --run-dir <run_dir> --iter 1

For fully automated runs (rare — code generation requires Codex), supply
...
def cmd_setup(args):
    ...
    print(json.dumps({
        ...
        "env": env_json,
        "next_step": (
            "Codex should now read iterv1/ncu_top.json and state.json, then write "
...
def cmd_close_iter(args):
    ...
    if not passed:
        print(json.dumps({
            ...
            "guidance": (
                "Codex should read bench.json['correctness'] and bench.stderr.txt, "
