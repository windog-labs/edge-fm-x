...
def audit_real_smolvla_frontend(
    ...
    *,
    ...
    prefix_module = _make_prefix_module(
        torch,
        policy,
        batch_keys=(
            image_key, OBS_STATE, OBS_LANGUAGE_TOKENS, OBS_LANGUAGE_ATTENTION_MASK
        ),
        make_attention_masks=make_att_2d_masks,
        tokens_key=OBS_LANGUAGE_TOKENS,
        masks_key=OBS_LANGUAGE_ATTENTION_MASK,
    )
...
def _make_prefix_module(
    torch: Any,
    policy: Any,
    *,
    batch_keys: tuple[str, ...],
    make_attention_masks: Any,
    tokens_key: str,
    masks_key: str,
) -> Any:
    """Share the upstream prefix computation across explicit input profiles."""
    ...
    class PrefixModule(torch.nn.Module):
        ...
        def forward(self, *values: Any) -> tuple[Any, ...]:
            batch = dict(zip(batch_keys, values, strict=True))
            ...
            prefix_embs, prefix_pad_masks, prefix_att_masks = (
                self.policy.model.embed_prefix(
                    images, image_masks, batch[tokens_key], batch[masks_key],
                    state=prepared_state,
                )
            )
            attention_masks = make_attention_masks(prefix_pad_masks, prefix_att_masks)
            ...
            return (prefix_pad_masks,) + tuple(
                ...
                for value in (cache[index]["key_states"], cache[index]["value_states"])
            )

    return PrefixModule().eval()
