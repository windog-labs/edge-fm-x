...
uint64_t fnv1a_64_py(const std::string& data) {
...
std::string fnv1a_hex_py(const std::string& data) {
    std::ostringstream oss;
    oss << std::hex << fnv1a_64_py(data);
    return oss.str();
}
...
std::string normalize_backend_target(const nlohmann::json& user_config) {
    if (user_config.contains("runtime") && user_config["runtime"].is_object()) {
        const std::string runtime_device = user_config["runtime"].value("device", std::string("cuda"));
        if (runtime_device == "cuda" || runtime_device == "gpu") {
            return "cuda";
        }
        if (runtime_device == "horizon") {
            return "horizon";
        }
        throw ConfigurationError(
            "Unsupported runtime.device for EdgeFM.from_model: " + runtime_device +
            ". Expected 'cuda', 'gpu', or 'horizon'");
    }
    return "cuda";
}
...
struct EmbeddingOpSpec {
    std::string weight_key = "model.embed_tokens.weight";
};

struct InjectEmbeddingOpSpec {
    bool enabled = false;
};

struct RMSNormOpSpec {
    int32_t layer_id = -1;
    std::string role;
    std::string weight_key;
};

struct QKVLinearOpSpec {
    int32_t layer_id = -1;
    std::string weight_key;
};

struct AttentionOpSpec {
    int32_t layer_id = -1;
};
...
struct SiLUAndMulOpSpec {
    int32_t layer_id = -1;
};

struct LMHeadOpSpec {
    std::string weight_key = "lm_head.weight";
};

struct DecoderLayerSpec {
    int32_t layer_id = -1;
    RMSNormOpSpec input_norm;
    QKVLinearOpSpec qkv;
    AttentionOpSpec attention;
    LinearOpSpec o_proj;
    RMSNormOpSpec post_norm;
    LinearOpSpec gate_up;
    SiLUAndMulOpSpec activation;
    LinearOpSpec down_proj;

    static DecoderLayerSpec hf(int32_t layer_id) {
        DecoderLayerSpec spec;
        const std::string prefix = "model.layers." + std::to_string(layer_id);
        spec.layer_id = layer_id;
        spec.input_norm = RMSNormOpSpec{
            layer_id,
            "input",
            prefix + ".input_layernorm.weight",
        };
        spec.qkv = QKVLinearOpSpec{
            layer_id,
            prefix + ".self_attn",
        };
        spec.attention = AttentionOpSpec{layer_id};
        spec.o_proj = LinearOpSpec{
            layer_id,
            "o_proj",
            prefix + ".self_attn.o_proj",
        };
        spec.post_norm = RMSNormOpSpec{
            layer_id,
            "post_attention",
            prefix + ".post_attention_layernorm.weight",
        };
        spec.gate_up = LinearOpSpec{
            layer_id,
            "gate_up_fused",
            prefix + ".mlp",
        };
        spec.activation = SiLUAndMulOpSpec{layer_id};
        spec.down_proj = LinearOpSpec{
            layer_id,
            "down_proj",
...
struct DecoderOnlyModelSpec {
    std::string name = "decoder_only_transformer";
    EmbeddingOpSpec embedding;
    InjectEmbeddingOpSpec inject_embedding;
    std::vector<DecoderLayerSpec> layers;
    RMSNormOpSpec final_norm{-1, "final", "model.norm.weight"};
    LMHeadOpSpec lm_head;

    void validate() const {
        if (layers.empty()) {
            throw ConfigurationError("DecoderOnlyModel requires at least one layer");
        }
        for (size_t i = 0; i < layers.size(); ++i) {
            if (layers[i].layer_id != static_cast<int32_t>(i)) {
                throw ConfigurationError("DecoderOnlyModel layer_id must be contiguous from 0");
            }
        }
    }

    nlohmann::json to_model_description() const {
        validate();
        nlohmann::json nodes = nlohmann::json::array();
        auto add_op = [&](const std::string& op_name,
            ...
            nodes.push_back({
                ...
                {"attrs", attrs.is_object() ? attrs : nlohmann::json::object()},
            });
        };
        ...
        add_op(
            ...
            {"token_ids"},
            {"hidden_states"},
            embedding.weight_key);
        if (inject_embedding.enabled) {
            add_op(
                "inject_embedding",
                "InjectEmbedding",
        ...
        for (const auto& layer : layers) {
            add_op(
                "layer_" + std::to_string(layer.layer_id) + "_input_norm",
                "RMSNorm",
                {"hidden_states"},
                {"norm_output"},
                layer.input_norm.weight_key,
                {{"layer_id", layer.layer_id}, {"role", layer.input_norm.role}});
            add_op(
                "layer_" + std::to_string(layer.layer_id) + "_qkv",
                "QKVLinear",
                {"norm_output"},
                {"qkv_output"},
                layer.qkv.weight_key,
                {{"layer_id", layer.layer_id}});
            add_op(
                "layer_" + std::to_string(layer.layer_id) + "_attention",
                "Attention",
                {"qkv_output", "kv_cache"},
                {"attn_output"},
                "",
                {{"layer_id", layer.layer_id}});
            add_op(
                "layer_" + std::to_string(layer.layer_id) + "_o_proj",
                "Linear",
                {"attn_output"},
                {"attn_projected"},
                layer.o_proj.weight_key,
                {{"layer_id", layer.layer_id}, {"role", layer.o_proj.role}});
            add_op(
                "layer_" + std::to_string(layer.layer_id) + "_attn_residual",
                "ResidualAdd",
                {"attn_projected", "hidden_states"},
                {"attn_residual"},
                "",
                {{"layer_id", layer.layer_id}, {"role", "attn"}});
            add_op(
                "layer_" + std::to_string(layer.layer_id) + "_post_norm",
                "RMSNorm",
                {"attn_residual"},
                {"post_norm"},
                layer.post_norm.weight_key,
                {{"layer_id", layer.layer_id}, {"role", layer.post_norm.role}});
            add_op(
                "layer_" + std::to_string(layer.layer_id) + "_gate_up",
                "Linear",
                {"post_norm"},
                {"mlp_gate_up"},
                layer.gate_up.weight_key,
                {{"layer_id", layer.layer_id}, {"role", layer.gate_up.role}});
            add_op(
                "layer_" + std::to_string(layer.layer_id) + "_activation",
                "SiLUAndMul",
                {"mlp_gate_up"},
                {"mlp_intermediate"},
                "",
                {{"layer_id", layer.layer_id}});
            add_op(
                "layer_" + std::to_string(layer.layer_id) + "_down_proj",
                "Linear",
                {"mlp_intermediate"},
                {"mlp_output"},
                layer.down_proj.weight_key,
                {{"layer_id", layer.layer_id}, {"role", layer.down_proj.role}});
            add_op(
                "layer_" + std::to_string(layer.layer_id) + "_mlp_residual",
                "ResidualAdd",
                {"mlp_output", "attn_residual"},
                {"hidden_states"},
                "",
                {{"layer_id", layer.layer_id}, {"role", "mlp"}});
        }
        add_op(
            "final_norm",
            "RMSNorm",
            {"hidden_states"},
            {"final_hidden_states"},
            final_norm.weight_key,
            {{"role", final_norm.role}});
        add_op(
            "lm_head",
            "LMHead",
            {"final_hidden_states"},
    ...
    static DecoderOnlyModelSpec hf(int32_t num_layers) {
        DecoderOnlyModelSpec model;
        for (int32_t layer_id = 0; layer_id < num_layers; ++layer_id) {
            model.layers.push_back(DecoderLayerSpec::hf(layer_id));
        }
        return model;
    }
};

std::filesystem::path write_internal_engine_config(
    const DecoderOnlyModelSpec& model_spec,
    const std::string& engine_json_path)
{
    const auto engine_config_path = std::filesystem::path(engine_json_path);
    const auto engine_config_dir = engine_config_path.parent_path();
    nlohmann::json user_config = nlohmann::json::parse(load_text_file(engine_config_path));
    if (!user_config.contains("prefill_model_path") ||
        user_config["prefill_model_path"].is_null() ||
        user_config["prefill_model_path"].get<std::string>().empty())
    {
        throw ConfigurationError("prefill_model_path is required in engine_json");
    }

    auto absolutize_model_path = [&](const char* key) {
        if (!user_config.contains(key) || user_config[key].is_null()) {
            return;
        }
        const std::string raw_path = user_config[key].get<std::string>();
        if (raw_path.empty()) {
            return;
        }
        const std::filesystem::path parsed(raw_path);
        if (parsed.is_absolute()) {
            return;
        }
        user_config[key] = (engine_config_dir / parsed).lexically_normal().string();
    };
    absolutize_model_path("prefill_model_path");
    absolutize_model_path("decode_model_path");

    const nlohmann::json model_description = model_spec.to_model_description();
    const auto prefill_model_path = std::filesystem::path(user_config["prefill_model_path"].get<std::string>());
    const auto model_config_path = prefill_model_path / "config.json";
    nlohmann::json model_config = nlohmann::json::parse(load_text_file(model_config_path));
    if (model_config.contains("text_config") && model_config["text_config"].is_object()) {
        model_config = model_config["text_config"];
    }

    const std::string backend_target = normalize_backend_target(user_config);
    const std::string model_description_hash = hash_model_description(model_description);

    const int32_t device_id = user_config.contains("runtime") && user_config["runtime"].is_object()
        ? user_config["runtime"].value("device_id", 0)
        : 0;
    const std::string hardware = current_backend_fingerprint(backend_target, device_id);
    const std::string backend_cache_key = fnv1a_hex_py(
        backend_target + "|" +
    ...
    nlohmann::json internal = {
        ...
        {"tuning_model_key", backend_cache_key},
    };
    if (backend_target == "cuda") {
        const ExecutionPlan plan = compile_model_description(model_description, model_config);
        internal["execution_plan"] = plan.to_json();
    }
    auto cached_artifact = BackendArtifactCache::instance().get_artifact(backend_cache_key);
    if (cached_artifact.has_value() && cached_artifact->backend == backend_target) {
        internal["backend_artifact"] = cached_artifact->to_json();
    }

    user_config["model_name"] = "layered_transformer";
    user_config["_edgefm_internal"] = std::move(internal);

    const auto temp_path = std::filesystem::temp_directory_path() /
        ("edgefm_from_model_" + model_description_hash + "_" + std::to_string(::getpid()) + ".json");
    std::ofstream output(temp_path);
    if (!output.is_open()) {
        throw ConfigurationError("Failed to create temporary engine config: " + temp_path.string());
    }
    output << user_config.dump(2);
    return temp_path;
}
...
PYBIND11_MODULE(edge_fm, m) {
    ...
    // ============================================================================
    // Python-facing model/layer specs
    // ============================================================================
    py::class_<EmbeddingOpSpec>(m, "EmbeddingOp", "Python 侧 Embedding 描述")
        .def(py::init<const std::string&>(),
             py::arg("weight_key") = "model.embed_tokens.weight")
        .def_readwrite("weight_key", &EmbeddingOpSpec::weight_key);

    py::class_<InjectEmbeddingOpSpec>(m, "InjectEmbeddingOp", "Python 侧外部 embedding 注入描述")
        .def(py::init<bool>(), py::arg("enabled") = true)
        .def_readwrite("enabled", &InjectEmbeddingOpSpec::enabled);

    py::class_<RMSNormOpSpec>(m, "RMSNormOp", "Python 侧 RMSNorm 描述")
        .def(py::init<int32_t, const std::string&, const std::string&>(),
             py::arg("layer_id") = -1,
             py::arg("role") = "",
             py::arg("weight_key") = "")
        .def_readwrite("layer_id", &RMSNormOpSpec::layer_id)
        .def_readwrite("role", &RMSNormOpSpec::role)
        .def_readwrite("weight_key", &RMSNormOpSpec::weight_key);

    py::class_<QKVLinearOpSpec>(m, "QKVLinearOp", "Python 侧 QKVLinear 描述")
        .def(py::init<int32_t, const std::string&>(),
             py::arg("layer_id") = -1,
             py::arg("weight_key") = "")
        .def_readwrite("layer_id", &QKVLinearOpSpec::layer_id)
        .def_readwrite("weight_key", &QKVLinearOpSpec::weight_key);

    py::class_<AttentionOpSpec>(m, "AttentionOp", "Python 侧 Attention 描述")
        .def(py::init<int32_t>(), py::arg("layer_id") = -1)
        .def_readwrite("layer_id", &AttentionOpSpec::layer_id);

    py::class_<LinearOpSpec>(m, "LinearOp", "Python 侧 Linear 描述")
        .def(py::init<int32_t, const std::string&, const std::string&>(),
             py::arg("layer_id") = -1,
             py::arg("role") = "",
             py::arg("weight_key") = "")
        .def_readwrite("layer_id", &LinearOpSpec::layer_id)
        .def_readwrite("role", &LinearOpSpec::role)
        .def_readwrite("weight_key", &LinearOpSpec::weight_key);

    py::class_<SiLUAndMulOpSpec>(m, "SiLUAndMulOp", "Python 侧 SiLUAndMul 描述")
        .def(py::init<int32_t>(), py::arg("layer_id") = -1)
        .def_readwrite("layer_id", &SiLUAndMulOpSpec::layer_id);

    py::class_<LMHeadOpSpec>(m, "LMHeadOp", "Python 侧 LMHead 描述")
        .def(py::init<const std::string&>(),
             py::arg("weight_key") = "lm_head.weight")
        .def_readwrite("weight_key", &LMHeadOpSpec::weight_key);

    py::class_<DecoderLayerSpec>(m, "DecoderLayer", "Python 侧 decoder layer 描述")
        .def(py::init<>())
        .def_readwrite("layer_id", &DecoderLayerSpec::layer_id)
        .def_readwrite("input_norm", &DecoderLayerSpec::input_norm)
        .def_readwrite("qkv", &DecoderLayerSpec::qkv)
        .def_readwrite("attention", &DecoderLayerSpec::attention)
        .def_readwrite("o_proj", &DecoderLayerSpec::o_proj)
        .def_readwrite("post_norm", &DecoderLayerSpec::post_norm)
        .def_readwrite("gate_up", &DecoderLayerSpec::gate_up)
        .def_readwrite("activation", &DecoderLayerSpec::activation)
        .def_readwrite("down_proj", &DecoderLayerSpec::down_proj)
        .def_static("hf", &DecoderLayerSpec::hf,
            py::arg("layer_id"),
            "创建一层符合 HuggingFace 命名约定的 decoder layer 描述");

    py::class_<DecoderOnlyModelSpec>(m, "DecoderOnlyModel", "Python 侧 decoder-only LLM 结构描述")
        .def(py::init<>())
        .def_readwrite("name", &DecoderOnlyModelSpec::name)
        .def_readwrite("embedding", &DecoderOnlyModelSpec::embedding)
        .def_readwrite("inject_embedding", &DecoderOnlyModelSpec::inject_embedding)
        .def_readwrite("layers", &DecoderOnlyModelSpec::layers)
        .def_readwrite("final_norm", &DecoderOnlyModelSpec::final_norm)
        .def_readwrite("lm_head", &DecoderOnlyModelSpec::lm_head)
        .def("validate", &DecoderOnlyModelSpec::validate)
        .def_static("hf", &DecoderOnlyModelSpec::hf,
            py::arg("num_layers"),
            "创建一个符合 HuggingFace 命名约定的 decoder-only 模型描述");
    ...
    py::class_<EdgeFM>(m, "EdgeFM", "EdgeFM 推理引擎")
        ...
        .def_static("from_model",
             [](const DecoderOnlyModelSpec& model, const std::string& engine_json) {
                 const auto temp_config_path = write_internal_engine_config(model, engine_json);
                 try {
                     auto engine = std::make_unique<EdgeFM>(temp_config_path.string());
                     std::error_code ec;
             ...
             py::arg("model"),
             py::arg("engine_json"),
             "从 Python 侧模型描述构造 EdgeFM 引擎。\n\n"
             "该接口会读取 engine_json 中的 prefill_model_path/decode_model_path，\n"
             "完成内部 graph/plan 构建、配置校验和权重加载。")
