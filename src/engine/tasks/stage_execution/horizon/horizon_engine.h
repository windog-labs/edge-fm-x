...
private:
    ...
    IRuntimeBackend& ensure_stage_runtime(const std::string& stage_name);
    ...
    std::unordered_map<std::string, std::unique_ptr<IRuntimeBackend>> stage_backends_;
    std::unordered_map<int32_t, CachedStageTensors> stage_tensor_cache_;
    std::optional<BackendArtifact> stage_artifact_;
