...
| Qwen3.5 | ✅ Runtime 已支持 | 支持 `examples/qwen3.5-0.8b`、`examples/qwen3.5-2b` text-only greedy generation；CUDA graph decode 已启用 |
...
详细性能表统一整理在 [edge_fm_benchmark_tables.md](doc/edge_fm_benchmark_tables.md)。README 只保留关键结论，避免多硬件、多模型矩阵把主页撑得太长。
...
| 平台 / 模型 | 关键结论 |
|---|---|
| RTX 3060 / Qwen2.5 LLM | EdgeFM CUDA graph 在当前矩阵中 `16/18` 个 shape 快于 TRT-Edge-LLM |
| A800 / Qwen2.5-VL | 多数长上下文 decode case 快于 TRT-Edge-LLM，完整表见文档 |
| Jetson Orin / Qwen2.5-VL-0.5B | 已记录 `512/32`、`1024/32` 端到端延迟，EdgeFM 总延迟约快 `27.7% - 32.8%` |
| Horizon J6M / SmolVLA-0.45B | 已记录 prefill 与 Action Expert decode 延迟，用于边缘平台回归 |

**Qwen3.5 EdgeFM-only 最新关键指标**（RTX 3060，CUDA graph on，text-only greedy，单位 ms；TRT 对比仍等待 TensorRT-Edge-LLM Qwen3.5 端口）：
