...
## Iteration 98 Candidate P0-PrecomputedSharedQK 2026-05-23T14:07:18+08:00

- Intent: resume the standalone long-loop and test a new P0 prefill algorithmic/layout candidate that stages per-token/head normalized `q/k` into shared memory inside `gated_delta_sequence_precomputed_kernel`. This preserves the Iter97 value tile, vectorized state decay/update, token-boundary barriers, state write/read phases, and per-column `k` accumulation order; it reduces repeated global `q/k` loads across the dot, state-update, and output phases.
- Rejected warmup candidate: `candidate_pair_columns` was standalone-correct (`output_max_abs=0.000488281`, `state_max_abs=5.96046e-08`) but slower (`1.414584 ms` vs Iter97 `1.207467 ms`) because halving dot/output workers lost more parallelism than q/k reuse saved.
- Standalone evidence: `candidate_shared_qk` passed the same CPU reference (`output_max_abs=0.000488281`, `state_max_abs=5.96046e-08`) and improved event time `1.207467 -> 0.699373 ms`. NCU duration improved `1.50 -> 0.953 ms`; achieved occupancy stayed high at `76.33%`, memory throughput `49.51%`, compute throughput `42.08%`, with remaining stalls still barrier/recurrence dominated.
- Production transfer: updated only `src/operators/qwen3_5/qwen3_5_ops.cu` precomputed P0 kernel to allocate shared `delta_s/key_s/query_s` and load normalized q/k once per token/head/value tile. Runtime scheduling, model state semantics, and Qwen2.5 code paths were not changed.
- Correctness: build passed; `EDGE_FM_BUILD_DIR=build pytest -q tests/operators/test_qwen3_5_runtime_ops.py -q` -> `15 passed`; `EDGE_FM_BUILD_DIR=build EDGE_FM_QWEN3_5_REGENERATE_DUMP=1 pytest -q tests/engine/test_qwen3_5_generate.py -s` -> `13 passed` for 0.8B and 2B.
- Benchmark: target graph-on runs improved versus Iter97. 0.8B p128/d32 `205.148 -> 197.393 ms`, prefill `31.394 -> 23.408 ms`; 2B p128/d32 `433.192 -> 425.986 ms`, prefill `44.654 -> 36.949 ms`; 2B p1024/d128 `1897.618 -> 1829.887 ms`, prefill `296.433 -> 229.596 ms`.
- Matrix: `iter98_shared_qk/matrix/` improved all six graph-on cases versus Iter93 and the targeted Iter97 validation cases. Graph-on 0.8B p128/p512/p1024 measured `735.102/795.678/875.656 ms`; 2B p128/p512/p1024 measured `1632.677/1720.722/1833.518 ms`. Graph-off 0.8B p128/p512/p1024 measured `779.852/840.038/920.492 ms`; 2B p128/p512/p1024 measured `1676.677/1766.954/1880.147 ms`.
- Qwen2.5 regression: current production diff is Qwen3.5-local. Performance smoke showed no Qwen2.5 regression: 0.5B p512/d32 graph-on `126.129 ms` vs doc baseline `126.609 ms`, 1.5B p512/d32 graph-on `346.369 ms` vs doc baseline `346.130 ms` (`+0.07%`, noise). `EDGE_FM_BUILD_DIR=build pytest -q tests/engine/test_qwen2_generate.py -k "token_alignment or kvcache" -s` -> `8 passed, 12 deselected`.
- Decision: accept. This is a material P0 current-layout improvement found by the standalone long-loop and transferred without runtime redesign. Next step is a fresh NSYS hotspot map; P0 may no longer dominate short prefill as heavily, while LMHead top1 remains memory-roofline and decode-chain microkernels remain tiny-grid limited.

## Iteration 99 Candidate P0-FuseUpdateOutput 2026-05-23T14:25:00+08:00

- Intent: after the Iter98 hotspot refresh, test one more P0 recurrence candidate that fuses the state-update and output-dot phases. The candidate keeps q/k shared staging, decay, delta computation, and per-column k accumulation order, but removes one block barrier and avoids re-reading updated state for the output dot.
- Standalone evidence: `candidate_fuse_update_output` passed the CPU reference (`output_max_abs=0.000488281`, `state_max_abs=5.96046e-08`) and improved event time `0.699373 -> 0.642836 ms`. NCU duration improved `0.953 -> 0.875 ms`, compute/memory throughput `55.85%`, achieved occupancy `73.62%`.
- Production correctness while testing: build passed; `tests/operators/test_qwen3_5_runtime_ops.py` -> `15 passed`; Qwen3.5 0.8B/2B fresh-dump generate -> `13 passed`.
- Benchmark: rejected. The standalone win did not transfer to end-to-end generate. 0.8B p128/d32 graph-on regressed `197.393 -> 200.697 ms`, 2B p128/d32 regressed `425.986 -> 428.665 ms`, and 2B p1024/d128 regressed `1829.887 -> 1863.548 ms`.
- Revert validation: restored Iter98 shared-qk separate vectorized state-update plus output-dot phases, rebuilt, and reran `tests/operators/test_qwen3_5_runtime_ops.py` (`15 passed`).
- Decision: reject and revert. The production compiler/runtime prefers the Iter98 vectorized state update and separate output phase even though the standalone fused schedule is faster. Keep Iter98 as the accepted P0 state.

## Iteration 100 Candidate P0-PrecomputedTile32AfterSharedQK 2026-05-23T14:30:58+08:00

- Intent: retest the old P0 precomputed larger value tile after Iter98 shared q/k staging changed the production balance. The candidate changes only `kGatedDeltaPrecomputedValueTile` from `16` to `32` in `src/operators/qwen3_5/qwen3_5_ops.cu`; runtime scheduling, model state semantics, and Qwen2.5 paths are untouched.
- Standalone evidence: `candidate_shared_qk` with `GD_VALUE_TILE=32,GD_THREADS=256` was CPU-reference correct and measured `0.620462 ms`; tile8 was also correct but slow at `1.370839 ms`, so only tile32 was transferred.
- Correctness: build passed; `EDGE_FM_BUILD_DIR=build pytest -q tests/operators/test_qwen3_5_runtime_ops.py -q` -> `15 passed`; `EDGE_FM_BUILD_DIR=build EDGE_FM_QWEN3_5_REGENERATE_DUMP=1 pytest -q tests/engine/test_qwen3_5_generate.py -s` -> `13 passed` for 0.8B and 2B.
- NCU: production `gated_delta_sequence_precomputed_kernel<bf16,true,32>` first matching 0.8B p128 launch measured `802.50 us`, compute throughput `35.90%`, memory throughput `48.65%`, achieved occupancy `38.20%`, eligible warps/scheduler `0.36`, no-eligible `71.25%`, grid/block `64/256`, registers/thread `40`. NCU flags small-grid and barrier stalls; the acceptance target is the recurrence-aware current-layout ceiling, not raw hardware peak.
- Target benchmark: graph-on improved 0.8B p128/d32 `197.393 -> 194.618 ms`, 2B p128/d32 `425.986 -> 423.158 ms`, and 2B p1024/d128 `1829.887 -> 1812.822 ms`; 2B p1024/d128 prefill improved `229.596 -> 211.914 ms`.
- Matrix: `iter100_precomputed_tile32/matrix/` improved all six graph-on and all six graph-off cases versus Iter98. Graph-on 0.8B p128/p512/p1024 measured `733.114/785.403/855.628 ms`; 2B measured `1629.689/1710.639/1814.472 ms`. Graph-off 0.8B p128/p512/p1024 measured `778.738/831.857/899.167 ms`; 2B measured `1672.990/1756.129/1860.306 ms`.
- Qwen2.5 regression: `EDGE_FM_BUILD_DIR=build pytest -q tests/engine/test_qwen2_generate.py -k "token_alignment or kvcache" -s` -> `8 passed, 12 deselected`; Qwen2.5 0.5B/1.5B p512/d32 graph-on smoke measured `127.077 ms` / `346.059 ms`, matching the docs baseline/noise band.
- Decision: accept. This is a Qwen3.5-local P0 current-layout improvement with full correctness and matrix evidence. The remaining P0 gap is recurrence/barrier/small-grid limited; further large gains should come from a new layout/algorithm search rather than another narrow retile.

## Iteration 101 Profile PostIter100Hotspot 2026-05-23T14:36:00+08:00

- Change: refreshed graph-off NSYS mapping after Iter100 tile32 acceptance.
- Evidence: `profiles/qwen3_5_nsys_action_iter100.txt` shows prefill `gated_delta_sequence_precomputed_kernel<bf16,true,32>` at `10.591 ms` (`49.5%`) across 18 launches, down from Iter99 pre-tile32 `13.290 ms`. Decode remains dominated by `lm_head_top1::stage1_kernel<bf16>` at `46.177 ms` (`25.5%`), followed by `gated_delta_sequence_from_ab_single_token_128_kernel` `4.923 ms`, fixed-hidden RMSNorm `3.342 ms`, FlashInfer attention `2.955 ms`, and add `2.155 ms`.
- Decision: profile-only. The remaining material local kernels are either recurrence/barrier/small-grid limited (P0/from_ab) or already documented memory-roofline (`lm_head_top1` stage1). The next production edit should come from a new algorithm/byte-reduction standalone search rather than another narrow retile.

## Iteration 102 Candidate P2-LMHeadTop1SharedHidden 2026-05-23T15:00:00+08:00

- Intent: test one narrow byte-reduction candidate for `lm_head_top1::stage1_kernel`: stage the single-token hidden vector in shared memory once per block so the 24 vocab-row warps reuse it locally. This targets redundant hidden-vector loads while preserving the exact per-row dot-product order and token tie-break semantics.
- Correctness while testing: build passed; `EDGE_FM_BUILD_DIR=build pytest -q tests/layers/test_linear.py -k top1 -q` -> `4 passed`; `EDGE_FM_BUILD_DIR=build pytest -q tests/operators/test_qwen3_5_runtime_ops.py -q` -> `15 passed`.
- Benchmark: 0.8B p128/d32 graph-on measured `194.416 ms` versus Iter100 `194.618 ms`, only `0.10%` faster and within normal run noise. This also repeats the already rejected Iter21 shared-hidden idea, which previously regressed the p128/d128 top1 gate.
- Decision: reject and revert. The candidate touches the common LMHead top1 path but gives no material kernel/end-to-end evidence beyond noise; keep the accepted no-shared-hidden memory-roofline top1 path.

## Iteration 103 Candidate P0-FuseUpdateOutputTile32 2026-05-23T15:21:00+08:00

- Intent: re-open the P0 fused update/output candidate only after Iter100 moved the accepted precomputed tile to `32`. Standalone `candidate_fuse_update_output_tile32` is correct and measured `0.544492 ms`, faster than the current `shared_qk_tile32` standalone `0.620462 ms`. Production transfer will stay Qwen3.5-local and must pass operator tests, 0.8B/2B fresh-dump alignment, and affected graph-on benchmarks before acceptance.
- Correctness while testing: build passed; `EDGE_FM_BUILD_DIR=build pytest -q tests/operators/test_qwen3_5_runtime_ops.py -q` -> `15 passed`; `EDGE_FM_BUILD_DIR=build EDGE_FM_QWEN3_5_REGENERATE_DUMP=1 pytest -q tests/engine/test_qwen3_5_generate.py -s` -> `13 passed`.
- Benchmark: the first valid sequential target case regressed. 0.8B p128/d32 graph-on measured `198.066 ms` versus Iter100 `194.618 ms`, with prefill `24.460 ms` versus Iter100 `20.800 ms`. Earlier parallel benchmark results were discarded because simultaneous GPU runs interfered.
- Revert validation: restored the Iter100 separate vectorized state-update plus output-dot path, rebuilt, reran Qwen3.5 operator tests (`15 passed`) and generate alignment (`13 passed`), then confirmed 0.8B p128/d32 graph-on recovered to `194.311 ms`.
- Decision: reject and revert. Even after tile32, the fused update/output standalone win does not transfer to full generate; keep Iter100 as the accepted P0 production state.

## Iteration 104 Candidate P0-PrecomputedTile32Threads512 2026-05-23T15:47:00+08:00

- Intent: run a precomputed-only launch-config candidate after Iter100 tile32. Standalone `shared_qk_tile32_t512` is correct and measured `0.605617 ms`, slightly faster than current `shared_qk_tile32_t256` `0.620462 ms`; `t128` was slower at `0.721159 ms`. Production transfer will add a separate precomputed sequence thread count so decode/generic `from_ab` paths remain at their accepted `256` threads.
- Correctness while testing: build passed; `tests/operators/test_qwen3_5_runtime_ops.py` -> `15 passed`; Qwen3.5 0.8B/2B fresh-dump generate -> `13 passed`.
- Benchmark: first target case regressed. 0.8B p128/d32 graph-on measured `195.215 ms` versus Iter100 `194.618 ms`, with prefill `21.528 ms` versus Iter100 `20.800 ms`.
- Revert validation: restored the accepted `256`-thread precomputed launch, rebuilt, and reran Qwen3.5 operator tests (`15 passed`).
- Decision: reject and revert. The standalone `512`-thread launch-config win does not transfer to full generate; keep Iter100 `tile32/thread256` as the accepted P0 launch shape.

## Iteration 105 Candidate P0-QMemOutputAlgebra 2026-05-23T16:10:00+08:00

- Intent: test a larger P0 algorithm candidate based on the identity `q^T(S + k*delta) = q^T S + (q^T k) * delta`. The standalone candidate computes `kv_mem`, `q_mem`, and `qk` in the delta loop, stores output before the vectorized state update, and avoids the post-update output state-read pass. It keeps the final recurrent state update path unchanged.
- Standalone evidence: `candidate_qmem_output_tile32` passed the CPU reference (`output_max_abs=0.000244141`, `state_max_abs=5.96046e-08`) and measured `0.503992 ms`, faster than current `shared_qk_tile32` `0.620462 ms`. Production risk is FP32 association/order change, so the first gate is fresh-dump token alignment.
- Production transfer: updated only `src/operators/qwen3_5/qwen3_5_ops.cu` precomputed P0 kernel. For each value column, the kernel now accumulates `kv_mem`, `q_mem`, and `qk` in the delta phase, stores `q_mem + delta * qk`, then runs the same vectorized recurrent state update. Runtime scheduling, model state semantics, decode kernels, and Qwen2.5 paths were not changed.
- Correctness: build passed before this iteration; after transfer, `EDGE_FM_BUILD_DIR=build pytest -q tests/operators/test_qwen3_5_runtime_ops.py -q` -> `15 passed`; `EDGE_FM_BUILD_DIR=build EDGE_FM_QWEN3_5_REGENERATE_DUMP=1 pytest -q tests/engine/test_qwen3_5_generate.py -s` -> `13 passed` for Qwen3.5 0.8B and 2B.
- NCU: production `gated_delta_sequence_precomputed_kernel<bf16,true,32>` first matching 0.8B p128 launch measured `705.70 us`, compute throughput `33.72%`, memory throughput `55.33%`, achieved occupancy `38.31%`, eligible warps/scheduler `0.41`, no-eligible `68.48%`, grid/block `64/256`, registers/thread `40`. This improves Iter100 duration `802.50 us -> 705.70 us` while NCU still flags the same small-grid/recurrence limit.
- Target benchmark: graph-on improved versus Iter100. 0.8B p128/d32 measured `192.986 ms` vs `194.618 ms`; 2B p128/d32 measured `421.638 ms` vs `423.158 ms`; 2B p1024/d128 measured `1803.137 ms` vs `1812.822 ms`.
- Matrix: `benchmarks/iter105_qmem_output_algebra/matrix/` improved all six graph-on and all six graph-off cases versus Iter100. Graph-on 0.8B p128/p512/p1024 measured `730.513/781.354/846.122 ms`; 2B measured `1628.511/1706.069/1805.514 ms`. Graph-off 0.8B p128/p512/p1024 measured `773.950/825.816/890.332 ms`; 2B measured `1672.714/1750.783/1851.745 ms`.
- Decision: accept. This is the first fused-output algebra variant that transfers to full Qwen3.5 generate after the earlier fused update/output attempts failed. The gain is modest end-to-end but real across the matrix and sizeable at the P0 kernel level. The remaining P0 gap is now even more clearly small-grid/recurrence limited; raw `95%` hardware peak remains an invalid target for this kernel.

## Iteration 106 ProfileAndCandidate P2-Qwen3_5DecodeMlpDownTactic 2026-05-23T16:35:00+08:00

- Profile refresh: captured Iter105 graph-off mapping and graph-on formal traces for 0.8B p128/d32. `profiles/qwen3_5_nsys_action_iter105.md` shows prefill `gated_delta_sequence_precomputed_kernel<bf16,true,32>` at `9.391 ms` (`46.5%` of prefill stage GPU time), down from Iter100 `10.591 ms`. Global graph-off kernel sum now has decode dense linear GEMV as the largest visible family: cuBLAS `gemv2T_kernel_val` `61.902 ms` (`30.8%`), `lm_head_top1::stage1_kernel` `47.668 ms` (`23.7%`), CUTLASS GEMM family `34.567 ms` (`17.2%`), P0 precomputed recurrence `9.426 ms` (`4.7%`), and decode `gated_delta_sequence_from_ab_single_token_128_kernel` `4.925 ms` (`2.4%`).
- Intent: follow the known linear path before writing a new kernel. Test whether adding a Qwen3.5/RTX3060 shape-specific decode `mlp_down` cuBLASLt record improves the dense linear hotspot without touching runtime code.
- Evidence: `tune_qwen_cublaslt.py` for Qwen3.5 0.8B decode `mlp_down`, `m=1`, found `algo_index=2` slightly faster than default: `0.034400 ms` vs `0.034816 ms` (`~1.2%` single-layer). Qwen3.5 decode `fused_gate_up` default was already best (`0.050128 ms` baseline, best baseline).
- Correctness while testing: operator table validation passed; Qwen3.5 0.8B/2B fresh-dump generate passed (`13 passed`).
- Benchmark: rejected. The added Qwen3.5 0.8B decode `mlp_down` record regressed the target end-to-end gate: 0.8B p128/d32 graph-on measured `193.411 ms` versus Iter105 `192.986 ms`. The candidate record was removed from `examples/config/platform/3060/operator_impl_table.json`, and table validation passed again.
- Decision: reject and revert. Per-layer cuBLASLt tactic wins below ~1-2% are not enough to survive CUDA graph end-to-end noise/interaction. Continue dense-linear work only with broader model/shape evidence or a larger algorithmic change; do not keep single-shape Qwen3.5 records unless they improve affected generate cases.

## Iteration 107 Candidate P2-Qwen3_5_2BDecodeLinearTactics 2026-05-23T16:50:00+08:00

- Intent: continue the known dense-linear path after Iter106 rejected the weak 0.8B single-shape record. This candidate keeps runtime untouched and adds only Qwen3.5/RTX3060 decode operator-table records for 2B shapes with clear local tuner margin: `mlp_down` `6144->2048` uses cuBLASLt `algo_index=1`, and generic Qwen3.5 linear `2048->2048` uses `algo_index=2`.
- Tuner evidence: Qwen3.5 2B decode `mlp_down` improved `0.093184 -> 0.080896 ms` (`15.2%` single-layer). Qwen3.5 2B decode generic linear `2048->2048` improved `0.035840 -> 0.032768 ms` for `linear_out` (`9.4%`) and `0.035760 -> 0.032768 ms` for `linear_z` (`9.1%`). Other tested Qwen3.5 0.8B/2B decode shapes were baseline-best or below the acceptance margin.
- Production transfer: updated only `examples/config/platform/3060/operator_impl_table.json`; no C++ kernel, runtime, scheduler, or model-state code changed. The records are keyed by `model_name=qwen3_5`, `hw_profile=cuda_sm86`, decode stage, and exact 2B shape signatures.
- Correctness: `python3 scripts/operator_table/validate_operator_tables.py --platform 3060` passed; `EDGE_FM_BUILD_DIR=build EDGE_FM_QWEN3_5_REGENERATE_DUMP=1 pytest -q tests/engine/test_qwen3_5_generate.py -s` -> `13 passed` for Qwen3.5 0.8B and 2B.
- Target benchmark: 2B graph-on target cases improved materially. p128/d32 moved `421.638 -> 409.160 ms`, p512/d128 moved `1706.069 -> 1655.072 ms`, and p1024/d128 moved `1803.137 -> 1753.474 ms`. A 0.8B p128/d32 quick gate measured `193.590 ms` versus Iter105 `192.986 ms`, within the expected noise band for shapes not targeted by these records.
- Matrix: `benchmarks/iter107_qwen3_5_2b_linear_tactics/matrix/` improved all six 2B graph-on/off cases versus Iter105 while leaving 0.8B effectively unchanged. Graph-on 2B p128/p512/p1024 measured `1578.680/1655.575/1756.324 ms` versus Iter105 `1628.511/1706.069/1805.514 ms` (`2.72%-3.06%` faster). Graph-off 2B p128/p512/p1024 measured `1618.779/1697.754/1797.839 ms` versus Iter105 `1672.714/1750.783/1851.745 ms` (`2.91%-3.22%` faster). Graph-on 0.8B p128/p512/p1024 measured `731.119/781.274/846.403 ms`, a `-0.01%` to `+0.08%` delta versus Iter105.
- Decision: accept. Unlike Iter106, this is a broad 2B shape-family tactic update with clear local tuner margins and full matrix transfer. The next step is a refreshed post-Iter107 hotspot map before choosing between remaining dense-linear, LMHead byte-reduction, or P1 decode-chain work.

## Iteration 108 ProfileAndCandidate P2-Qwen3_5_2BSelfAttnVTactic 2026-05-23T17:10:00+08:00

- Profile refresh: captured post-Iter107 0.8B p128/d32 graph-off mapping and graph-on formal traces plus a 2B p128/d32 graph-off mapping trace. The 0.8B stage map remains unchanged because Iter107 targets only 2B shape signatures: prefill P0 is still `9.391 ms` (`46.5%` of prefill stage), with `lm_head_top1` prefill `1.492 ms`. The 2B graph-off global kernel summary now shows dense GEMV families still dominate: cuBLAS `gemv2T_kernel_val` `165.320 ms` (`39.7%`), `lm_head_top1::stage1_kernel` `95.245 ms` (`22.9%`), cuBLAS GEMVX `60.161 ms` (`14.4%`), CUTLASS GEMM `33.648 ms` (`8.1%`), and P0 precomputed recurrence only `9.392 ms` (`2.3%`).
- Intent: fill a small 2B tuner blind spot before moving away from dense-linear tactics. Tune Qwen3.5 2B decode `self_attn_k`, `self_attn_v`, `linear_a`, and `linear_b`; only transfer if local margin is large enough and the affected p128/d32 graph-on gate improves beyond noise.
- Tuner evidence: `linear_a` baseline was best (`0.010496 ms`), `linear_b` `algo_0` was only `0.16%` faster (`0.010304 -> 0.010288 ms`), and `self_attn_k` `algo_11` was only `0.12%` faster (`0.013024 -> 0.013008 ms`). `self_attn_v` `algo_11` was the only plausible candidate at `0.013136 -> 0.012576 ms` (`4.45%` local).
- Correctness while testing: adding the temporary Qwen3.5/RTX3060 decode `linear` `2048->512` `algo_index=11` record passed operator-table validation and Qwen3.5 0.8B/2B fresh-dump generate (`13 passed`).
- Benchmark: rejected. 2B p128/d32 graph-on measured `409.071 ms` versus Iter107 `409.160 ms`, only `0.02%` faster and fully within noise. The temporary `2048->512` record was removed and operator-table validation passed again.
- Decision: reject and revert. Remaining cuBLASLt single-shape tactics with local margins below roughly `5%` are not worth production table churn unless they show a clear full-generate win. Next step should be either a larger dense-linear algorithm/shape-family opportunity or an LMHead/top1 byte-reduction or decode-chain standalone candidate.

## Iteration 109 Candidate P2-Qwen3_5_2BExplicitLinearTacticSearch 2026-05-23T17:25:00+08:00

- Intent: after Iter108 closed the small `2048->512` tactic, test whether the largest remaining Qwen3.5 2B decode dense-linear shapes have hidden cuBLASLt explicit configs beyond the heuristic candidates. This is a tuner-only pass with no production table change unless a candidate beats baseline with a clear margin.
- Evidence: explicit cuBLASLt search found baseline still best for all three checked high-weight shapes. `fused_gate_up 2048->12288` baseline `0.154624 ms` remained best; best explicit was slower at `0.157696 ms`. `qwen3_linear_qkv 2048->6144` baseline `0.081216 ms` remained best; best explicit was `0.084992 ms`. `self_attn_q 2048->4096` baseline `0.057344 ms` remained best; best explicit was `0.061136 ms`.
- Decision: no production change. The pure cuBLASLt tactic path for the largest remaining 2B decode linear shapes is at the measured library tactic ceiling for this phase. Further P2 gains require a larger algorithmic change, such as reducing required LMHead bytes or fusing/restructuring a decode chain, not another operator-table record.

## Iteration 110 Profile P2-LMHeadTop1Roofline 2026-05-23T17:35:00+08:00

- Intent: refresh NCU evidence for the current Qwen3.5 2B default greedy `lm_head_top1::stage1_kernel` before attempting further LMHead work.
- Evidence: NCU on the first matching 2B `stage1_kernel` launch measured duration `2.9808 ms`, DRAM throughput `97.60%`, compute-memory throughput `97.60%`, SM throughput `61.85%`, active warps `97.79%` of peak sustained active, and issue active `28.91%`. Report: `profiles/ncu/qwen3_5_2b_lm_head_top1_stage1_iter110.ncu-rep`; raw CSV: `profiles/ncu/qwen3_5_2b_lm_head_top1_stage1_iter110_raw.csv`.
- Decision: no production change. The current LMHead top1 stage1 path is at the memory-bandwidth ceiling for this phase. Future LMHead work must reduce bytes read or change the search algorithm; launch-shape/shared-hidden retuning is closed.

## Iter111 - Candidate: decode single-token GatedDelta + gated RMSNorm fusion
- Intent: fuse Qwen3.5 decode-only `gated_delta_sequence_from_ab` producer with immediately following gated RMSNorm for `key_dim=value_dim=128`, keeping the change inside Qwen3.5 operator/model code and preserving prefill/general paths.
- Candidate methods: memory.kernel_fusion, compute.reduction/warp_shuffle for hidden=128 RMSNorm, latency.launch_count reduction.
- Production transfer: added a Qwen3.5-only fused single-token kernel in `src/operators/qwen3_5/qwen3_5_ops.cu`, a debug binding/operator equivalence test, and a model-side decode fast path that writes directly to `gated_norm_out`. Prefill and generic decode fallback paths are unchanged.
- Correctness: build passed; `EDGE_FM_BUILD_DIR=build pytest -q tests/operators/test_qwen3_5_runtime_ops.py -q` -> `16 passed`; `EDGE_FM_BUILD_DIR=build EDGE_FM_QWEN3_5_REGENERATE_DUMP=1 pytest -q tests/engine/test_qwen3_5_generate.py -s` -> `13 passed`.
- Benchmark: short gates measured 0.8B p128/d32 graph-on `192.630 ms` vs Iter107 `193.590 ms`, and 2B p128/d32 graph-on `409.155 ms` vs Iter107 `409.160 ms`. Full matrix measured graph-on 0.8B p128/p512/p1024 `729.895/781.488/845.920 ms` and 2B `1577.651/1655.172/1755.485 ms`; graph-off 0.8B `772.554/824.057/890.105 ms` and 2B `1618.101/1696.311/1797.104 ms`.
- Decision: accept. The gain is small but positive/neutral across the full matrix, removes one decode launch/data round trip for the common Qwen3.5 shape, and stays confined to model-local Qwen3.5 code. Further meaningful decode-chain gains likely require a larger standalone fusion of adjacent decode ops, not more tiny launch-only edits.

## Iter112 - Candidate: decode conv1d + GatedDelta + gated RMSNorm fusion
- Intent: fuse Qwen3.5 decode-only depthwise conv1d kernel4 state update with the accepted Iter111 GatedDelta + gated RMSNorm fusion for `seq_len=1,key_dim=value_dim=128,kernel=4`. This targets one fewer CUDA graph node and removes the `conv_out` global write/read while keeping prefill and generic decode fallbacks unchanged.
- Candidate methods: memory.kernel_fusion, memory.coalesced_access for q/k/v conv outputs kept in registers/shared, latency.launch_count reduction.
- Production transfer: added a Qwen3.5-only fused conv1d + GatedDelta + gated RMSNorm decode kernel, debug binding/operator equivalence test, and model-side fast path that skips `conv_out` materialization for the supported decode shape.
- Correctness: build passed; `EDGE_FM_BUILD_DIR=build pytest -q tests/operators/test_qwen3_5_runtime_ops.py -q` -> `17 passed`; `EDGE_FM_BUILD_DIR=build EDGE_FM_QWEN3_5_REGENERATE_DUMP=1 pytest -q tests/engine/test_qwen3_5_generate.py -s` -> `13 passed`.
- Benchmark: short gates measured 0.8B p128/d32 graph-on `192.596 ms` vs Iter111 `192.630 ms`, and 2B p128/d32 graph-on `408.857 ms` vs Iter111 `409.155 ms`. Full matrix measured graph-on 0.8B p128/p512/p1024 `729.227/780.646/845.858 ms` and 2B `1576.995/1654.142/1754.552 ms`; graph-off 0.8B `769.482/822.184/887.597 ms` and 2B `1616.000/1693.494/1793.715 ms`.
- Decision: accept. The improvement is small but consistent across graph-on/off matrix and keeps runtime untouched. This likely closes the simple local decode conv/GatedDelta/RMSNorm launch-fusion tier; larger gains require fusing GEMV-producing inputs, output projection restructuring, or a new LMHead byte-reduction algorithm.

## Iter113 - Candidate: fixed-hidden RMSNorm 2048 thread-count retune
- Intent: post-Iter112 NSYS still shows Qwen3.5 2B non-gated RMSNorm hidden=2048 as a small but visible decode component (`~4.1 ms`, many tiny launches). Test a Qwen3.5-local launch retune from 256 to 512 threads for fixed hidden=2048 while leaving hidden=1024/gated/generic paths unchanged.
- Candidate methods: compute.launch_config, compute.reduction, latency.warp_shuffle_sync not applicable because this kernel uses block-wide reduction over 2048.
- Correctness while testing: build passed; `EDGE_FM_BUILD_DIR=build pytest -q tests/operators/test_qwen3_5_runtime_ops.py -q` -> `17 passed`.
- Benchmark: rejected by the affected 2B p128/d32 graph-on short gate. Iter113 measured `409.124 ms` versus Iter112 `408.857 ms`; decode step moved `12.115 -> 12.118 ms`. The candidate was reverted before continuing.
- Decision: reject and revert. The existing fixed-hidden 2048 RMSNorm 256-thread launch remains the accepted local configuration.

## Iter114 - Candidate: Qwen3.5 2B full-attention decode cublasLt table records
- Intent: after LMHead top1 was confirmed DRAM-roofline and the largest linear-attention projection scans were baseline-best, close the remaining low-risk Qwen3.5 2B full-attention decode cublasLt table gaps without touching runtime or kernels.
- Tuner evidence: `qwen3_linear_qkv 2048->6144` and `self_attn_q 2048->4096` were baseline-best. `self_attn_o 2048->2048` improved `0.035136 -> 0.032768 ms` with `algo_index=2`; `self_attn_k/v 2048->512` improved to `0.012528 ms` with `algo_index=11`.
- Production transfer: added two Qwen3.5/RTX3060 decode operator-table records: `attention_output 2048->2048 algo_index=2` and `linear 2048->512 algo_index=11`. Also added `qwen3_self_attn_o` to `scripts/tune/tune_qwen_cublaslt.py` so future scans use full-attention layer3 instead of Qwen3.5 linear-attention layer0.
- Correctness: `python3 scripts/operator_table/validate_operator_tables.py --platform 3060` passed; `python3 -m py_compile scripts/tune/tune_qwen_cublaslt.py` passed; `EDGE_FM_BUILD_DIR=build EDGE_FM_QWEN3_5_REGENERATE_DUMP=1 pytest -q tests/engine/test_qwen3_5_generate.py -s` -> `13 passed` for Qwen3.5 0.8B and 2B.
- Benchmark: 2B graph-on gates improved versus Iter112. p128/d32 measured `407.973 ms` vs `408.857 ms`; p128/d128 measured `1572.494 ms` vs `1576.995 ms`; p512/d128 measured `1648.430 ms` vs `1654.142 ms`; p1024/d128 measured `1748.435 ms` vs `1754.552 ms`. Decode step improved to `12.091/12.119/12.163/12.171 ms` for these four gates.
- Decision: accept. This is table-only, Qwen3.5/SM86 scoped, and no Qwen2.5 runtime path changes were made. It likely closes the small visible full-attention o/k/v table opportunity; further material wins need an exact LMHead byte-reduction/search algorithm or a larger GEMV/fused-projection design.

## Iter115 - Candidate: enable existing SM86 decode fused SwiGLU path
- Intent: test whether the existing TensorRT-LLM SM80 fused-MoE SwiGLU fast path can be safely enabled on RTX 3060/SM86 for Qwen3.5 decode GateUp + SiLU + Mul, which remains a material GEMV/activation chain after Iter114.
- Correctness evidence: with `EDGE_FM_DECODE_SWIGLU_ALLOW_SM86=1`, `tests/operators/test_fused_gate_up_activation.py::test_decode_fused_gate_up_swiglu_matches_two_stage_output` passed, so the isolated layer output matched the two-stage path under the test tolerance.
- Token-alignment gate: rejected. With `EDGE_FM_DECODE_SWIGLU_ALLOW_SM86=1 EDGE_FM_QWEN3_5_REGENERATE_DUMP=1`, `tests/engine/test_qwen3_5_generate.py -s` failed Qwen3.5 2B token alignment at decode token index 8 (`118822` vs reference `97273`) in both regular and CUDA graph cases. Later tests hit OOM after the failed run, so no benchmark was attempted.
- Decision: reject and keep SM86 fused SwiGLU disabled by default. The existing comment about prior SM86 token regressions still applies. Any future SwiGLU fusion for RTX 3060 needs a new Qwen3.5-safe kernel or a stricter numerical contract, not simply enabling the SM80 TRT-LLM path.

## Iter116 - Candidate: Qwen3.5 0.8B safe decode linear cublasLt records
- Intent: refresh post-Iter114 hotspot evidence and close the remaining low-risk 0.8B dense-linear operator-table path without changing runtime or CUDA kernels.
- Profile refresh: 2B p128/d32 graph-off Iter116 mapping measured `427.961 ms` total, `37.885 ms` prefill, and `12.572 ms` decode step. NSYS still shows decode `lm_head_top1::stage1_kernel<bf16>` as the largest single kernel (`92.265 ms`, `24.2%`), GateUp GEMV as the largest repeated dense family (`~4.68 ms/layer`), fused conv/GatedDelta/gated RMSNorm `5.922 ms`, and fixed-hidden RMSNorm 2048 `4.122 ms`.
- 2B GateUp evidence: `fused_gate_up 2048->12288` all-mode cublasLt search found baseline still fastest (`0.154624 ms`); best non-baseline candidates were slower. No 2B GateUp table record was added.
- 0.8B tuner evidence: `fused_gate_up`, `qwen3_linear_qkv`, `qwen3_linear_z`, `self_attn_q`, `self_attn_k`, and `self_attn_v` were baseline-best. `mlp_down 3584->1024` improved `0.034816 -> 0.032752 ms`, `linear 2048->1024` improved `0.024512 -> 0.021504 ms`, and `attention_output 2048->1024` improved `0.023552 -> 0.021504 ms` with the same explicit cublasLt config.
- Correctness isolation: the three 0.8B records were each individually token-safe, but `mlp_down+attention_output` and all three records together diverged at decode token index 8. The accepted safe pair is `mlp_down 3584->1024` plus generic `linear 2048->1024`; `attention_output 2048->1024` is rejected in combination.
- Production transfer: updated only `examples/config/platform/3060/operator_impl_table.json` and fixed `scripts/tune/tune_qwen_cublaslt.py` so `qwen3_self_attn_o` uses `full_attention_dim -> hidden`, which matters for 0.8B where `full_attention_dim=2048` and `hidden=1024`.
- Correctness: operator-table validation passed; `python3 -m py_compile scripts/tune/tune_qwen_cublaslt.py` passed; `EDGE_FM_BUILD_DIR=build EDGE_FM_QWEN3_5_REGENERATE_DUMP=1 pytest -q tests/engine/test_qwen3_5_generate.py -s` -> `13 passed` for Qwen3.5 0.8B and 2B.
- Benchmark: 0.8B graph-on gates improved versus Iter112. p128/d32 moved `192.596 -> 189.299 ms`; p128/d128 moved `729.227 -> 715.516 ms`; p512/d128 moved `780.646 -> 766.434 ms`; p1024/d128 moved `845.858 -> 830.181 ms`.
- Decision: accept. This is table-only, Qwen3.5/SM86 scoped, and no Qwen2.5 runtime or operator path changes were made. The scanned low-risk 0.8B/2B cublasLt tactic path is now closed; further material gains need a larger LMHead byte-reduction/search algorithm or a new GEMV/fused-projection design.

## Iter117 - Profile/Blocker: LMHead exact top1 byte-reduction feasibility
- Intent: after Iter116 closed the safe cublasLt table path, test whether the largest remaining single kernel, `lm_head_top1::stage1_kernel<bf16>`, has an exact pruning route that could reduce weight bytes without changing greedy token semantics.
- Hotspot refresh: 0.8B p128/d32 graph-off Iter116 mapping shows decode `lm_head_top1::stage1_kernel<bf16>` at `46.174 ms` (`26.3%`), fused conv/GatedDelta/gated RMSNorm `5.944 ms` (`3.4%`), fixed-hidden RMSNorm `3.733 ms` (`2.1%`), FlashInfer decode attention `2.958 ms` (`1.7%`), and residual add `2.296 ms` (`1.3%`). 2B p128/d32 graph-off Iter116 mapping shows LMHead top1 `92.265 ms` (`24.2%`) and GateUp GEMV as the largest repeated dense family; 2B fused_gate_up all-mode cublasLt search remains baseline-best.
- Feasibility check: for 0.8B first 6 generated steps and 2B first 3 generated steps, compute an optimistic Cauchy row-norm upper bound `||hidden|| * ||lm_head_row||` using the final true best logit as if it were known before scanning. This is an upper bound on how much an exact row-norm pruning algorithm could skip.
- Evidence: both models pruned `0 / 248320` vocab rows in every sampled step, even with the perfect final best logit. A practical seed from the top-4096 row norms also pruned `0` rows. Artifacts: `analysis/qwen3_5_0p8b_lmhead_prune_feasibility_iter117.json` and `analysis/qwen3_5_2b_lmhead_prune_feasibility_iter117.json`.
- Decision: record blocker. Exact top1 pruning by simple norm bounds has no byte-reduction headroom for these Qwen3.5 prompts. Since Iter110 already measured LMHead stage1 at DRAM `97.60%`, the remaining LMHead path is at the current exact-algorithm ceiling; further gains require a qualitatively different exact search/indexing method or an accepted approximation/quantization contract, which is outside the current narrow runtime-preserving loop.

## Iter118 - Standalone Rejection: LMHead BF16x2 / launch-shape sweep

- Intent: before touching production `src/layers/linear.cu`, build a standalone exact LMHead top1 repro to test a low-risk BF16x2 load path and `16/24/32` warps-per-block variants for Qwen3.5 0.8B and 2B shapes.
- Artifact: `deliverables/kernel_opt/qwen3_5_lmhead_top1_iter118/`, with CUDA extension source, benchmark harness, source ledger, and `benchmarks/lmhead_top1_iter118_baseline.json`.
- Correctness: all variants matched the scalar24 production-equivalent token and `torch.mv(weight.float(), hidden.float()).argmax()` for both standalone shapes.
- Benchmark:
  - 0.8B `vocab=248320, hidden=1024`: scalar24 `1.4995 ms`; best `bf162_w24` `1.4970 ms` (`~0.2%` faster).
  - 2B `vocab=248320, hidden=2048`: scalar24 `2.9854 ms`; best `bf162_w32` `2.9786 ms` (`~0.2%` faster).
- Decision: reject production migration. The candidate is exact but flat, below the migration threshold, and supports the Iter110/117 conclusion that current exact LMHead top1 is at the DRAM-bound current-algorithm ceiling. No `src/` production code was changed for this candidate.

## Iter119 - Rejection: SM86 decode fused SwiGLU 0.8B-only recheck

- Intent: revisit the existing `EDGE_FM_DECODE_SWIGLU_ALLOW_SM86=1` path narrowly for Qwen3.5 0.8B. Iter115 rejected it because 2B token alignment diverged, but the log did not isolate whether 0.8B was safe or useful.
- Correctness: with `EDGE_FM_DECODE_SWIGLU_ALLOW_SM86=1 EDGE_FM_QWEN3_5_REGENERATE_DUMP=1`, `test_qwen3_5_generate_token_alignment[0.8b]` passed (`1 passed`). This does not clear the path globally because 2B remains a known token-alignment failure.
- Benchmark: 0.8B p128/d32 graph-on measured baseline `189.794 ms` and env-enabled `189.778 ms`, both with warmup 1 / runs 5. The delta is `~0.01%`, i.e. noise.
- Decision: reject production migration. Enabling the existing SM80 TRT-LLM fused SwiGLU path for SM86 remains default-off: 2B is unsafe, and the 0.8B-only signal is too small to justify a special-case production branch.

## Iter120 - Rejection: existing CUTLASS prefill SwiGLU fusion for Qwen3.5

- Intent: test the existing prefill `cutlass_prefill_swiglu` fast path under `EDGE_FM_PREFILL_SWIGLU_FUSION=1`. This path is separate from the decode SM86 fused SwiGLU blocker and affects prefill only, so it is a plausible low-risk candidate if token alignment and p512/p1024 prefill cases improve.
- Correctness: `EDGE_FM_PREFILL_SWIGLU_FUSION=1 EDGE_FM_QWEN3_5_REGENERATE_DUMP=1` passed both regular token alignment and CUDA graph enabled token alignment for Qwen3.5 0.8B/2B (`2 passed` in each targeted test).
- Benchmark:
  - 0.8B p512/d128 graph-on: baseline rerun `768.245 ms`, env-enabled `769.714 ms`; prefill `64.352 -> 65.756 ms`.
  - 2B p512/d128 graph-on: accepted baseline `1648.430 ms`, env-enabled `1652.397 ms`; prefill `103.371 -> 106.761 ms`.
- Artifact: `benchmarks/iter120_prefill_swiglu_env/summary.json`.
- Decision: reject production migration. The path is correctness-clean but regresses representative 0.8B and 2B graph-on cases, so the env default remains off and no model/runtime switch is added.

## Iter121 - Acceptance: Qwen3.5 decode GateUp+SwiGLU warp kernel

- Intent: after the existing SM86 TRT-LLM SwiGLU path was rejected, test a Qwen3.5-safe BF16 decode GateUp+SwiGLU warp GEMV kernel that computes the gate/up dot products and SwiGLU output in one launch. Keep it out of the generic default selection and enable it only through Qwen3.5/SM86 operator-table records.
- Standalone evidence: `deliverables/kernel_opt/qwen3_5_gateup_swiglu_iter121/benchmarks/gateup_swiglu_iter121.json` shows the standalone fused path is faster than the torch two-step reference on both visible shapes: 0.8B hidden/intermediate `1024/3584` best `0.0479 ms` vs `0.0658 ms`, and 2B `2048/6144` best `0.1522 ms` vs `0.1719 ms`. This was used only as a direction signal; token alignment remained the production gate.
- Production transfer: added a non-default `edgefm_decode_swiglu_warp` implementation in `src/operators/fused_gate_up_activation_op.cu` and Qwen3.5/RTX3060 decode operator-table records for `1024->3584` and `2048->6144` GateUp shapes. The default implementation explicitly skips this path unless selected by table or `EDGE_FM_DECODE_SWIGLU_WARP_DEFAULT`, so Qwen2.5/default paths do not silently change.
- Correctness: build passed; `python3 scripts/operator_table/validate_operator_tables.py --platform 3060` passed; `EDGE_FM_BUILD_DIR=build pytest -q tests/operators/test_qwen3_5_runtime_ops.py -q` -> `17 passed`; `EDGE_FM_BUILD_DIR=build EDGE_FM_QWEN3_5_REGENERATE_DUMP=1 pytest -q tests/engine/test_qwen3_5_generate.py -s` -> `13 passed`.
- Benchmark: accepted target graph-on gates improved versus the latest accepted baselines. 0.8B p128/d32 moved `189.299 -> 188.130 ms`, 0.8B p128/d128 `715.516 -> 711.708 ms`, 2B p128/d32 `407.973 -> 405.948 ms`, and 2B p128/d128 `1572.494 -> 1563.125 ms`. Decode step averages improved to `5.430/5.448/12.020/12.046 ms` respectively.
- Artifact: `benchmarks/iter121_gateup_warp/summary.json`.
- Decision: accept. The gain is small but consistent across 0.8B/2B and short/long decode gates, the correctness gate is clean, and the implementation is selected only by exact Qwen3.5/SM86 table records. The next local work is a post-Iter121 Qwen2.5 guard plus a refreshed hotspot check before deciding whether any remaining local kernel path is still open.

## Iter122 - Profile/Blocker: Qwen3.5 GateUp+SwiGLU warp ceiling

- Intent: because Iter121 made `edgefm_decode_swiglu_warp_kernel<16>` the top visible 2B graph-off hotspot, measure whether the accepted custom GateUp+SwiGLU GEMV kernel still has local launch/config tuning headroom.
- Guard: after Iter121 touched the shared fused-gate-up operator file, `EDGE_FM_BUILD_DIR=build pytest -q tests/engine/test_qwen2_generate.py -k "token_alignment or kvcache" -s` passed (`8 passed, 12 deselected`), confirming existing Qwen2.5 token/KV paths were not affected.
- NSYS: post-Iter121 2B p128/d32 graph-off mapping trace shows `edgefm_decode_swiglu_warp_kernel<16>` at `110.853 ms` across `744` launches (`29.3%` of decode GPU time), followed by `lm_head_top1::stage1_kernel<bf16>` at `92.260 ms` (`24.4%`), fused conv/GatedDelta/gated RMSNorm at `5.921 ms`, and fixed-hidden RMSNorm at `4.144 ms`. Report: `profiles/qwen3_5_nsys_action_iter121.md`.
- NCU: first matching 2B decode launch measured `153.70 us`, memory/DRAM throughput `95.34%`, compute throughput `23.99%`, achieved occupancy `93.18%`, theoretical occupancy `100%`, active warps/scheduler `11.12`, eligible warps/scheduler `0.34`, block/grid `512/384`, and `33` registers/thread. Report: `profiles/ncu/qwen3_5_2b_decode_swiglu_warp_iter122.ncu-rep`; CSV: `profiles/ncu/qwen3_5_2b_decode_swiglu_warp_iter122_details.csv`.
- Decision: no production change. Under the current exact BF16 GEMV algorithm, this kernel is at the documented memory-bandwidth ceiling (`>=95%` DRAM). Further material improvement requires reducing bytes read or changing the GEMV/search contract, such as a custom quantized/approximate path, weight compression, or a larger fused-projection design. That is outside the current narrow local-kernel loop.
