...
- Latest strategy convergence: `2026-05-12`, 3060 tuning no longer treats more CUTLASS retuning or a new handwritten CUTLASS-style kernel as the active route. `nsys` evidence now consistently shows that TRT-Edge-LLM wins on this platform with TensorRT compiler-generated closed kernels such as Myelin/XMMA/FcCast-style tactics, and those kernels materially outperform the current source-visible/open-kernel paths available in this repo. Because they are not directly reusable as normal EdgeFM operator impls, the practical route on 3060 is a reviewed, default-off TensorRT subgraph/subengine bridge for selected prefill modules. This is a hardware/toolchain kernel-availability limit on 3060, not an EdgeFM architecture failure.
