...
- Myelin/XMMA direct reuse check:
  ...
  - next valid action is an env-gated FP16 TensorRT-backed MLP bridge prototype using runtime-bound existing weights, or a genuinely new source-visible third-party runner with operator evidence; do not add fake `myelin`/`xmma` records or another small classic CUTLASS layout variant
