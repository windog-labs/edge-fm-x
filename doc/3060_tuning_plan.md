...
1. Prototype an env-gated FP16 TensorRT MLP bridge for prefill only, using one runtime-weight engine per MLP shape and binding existing GateUp/DownProj GPU weights as inputs. The first production-style proof must cover layer-output correctness, CUDA graph capture/replay behavior, and `3B / 2048x32` target-slice latency before any default-on decision.
