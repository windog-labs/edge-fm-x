...
1. Prototype an env-gated TensorRT MLP bridge for prefill only, using one runtime-weight engine per MLP shape, binding existing BF16 GateUp/DownProj GPU weights in EdgeFM's resident layout, casting internally to FP16 for TensorRT XMMA, and returning BF16 output. The first production-style proof must cover layer-output correctness, CUDA graph capture/replay behavior, and `3B / 2048x32` target-slice latency before any default-on decision.
