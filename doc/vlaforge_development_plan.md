...
| P10 | 真实 OpenVLA/SmolVLA/DiffusionDrive 等 L2–L4 | SmolVLA 真实 Host-CUDA L4 已完成；DiffusionDrive 真实 L3、OpenVLA 真实 L2 完成；DiffusionDrive L4 与 OpenVLA L3/L4 待完成 |
...
2026-07-25 当前结果：offline Python 188 passed/6 opt-in skipped；real
SmolVLA L4、DiffusionDrive L2/L3 opt-in 均各 1 passed；clean C++ Release、CPU 7/7 CTest、CUDA/AOTI
...
1. DiffusionDrive 已完成真实 L2/L3；继续完成 five-Region verified bundle、
   checkpoint candidates/scores generated C++ parity 和真实 L4；
2. OpenVLA：在已通过真实 L2 基础上完成分 Region L3，12GB 允许时推进 L4；
3. 为已完成的 SmolVLA L4 补齐 Host CUDA benchmark、消融、nsys/ncu 与
   10k+ Run soak；
4. 选择 Octo/GR00T 与 AutoVLA/ReCogDrive 做 frozen-core held-out；
5. Orin 环境就绪后执行模型专属 SM87 artifact 和真机验证；standalone
