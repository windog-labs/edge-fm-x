#!/usr/bin/env python3
"""
Qwen2.5-1.5B-Instruct 模型推理示例

此示例展示了如何使用 EdgeFM Python 接口进行 Qwen2.5-1.5B-Instruct 模型的推理。
支持纯文本推理。
"""
...
# 添加项目根目录到路径（使用 build/install/python）
...
def main():
    ...
    print(f"EdgeFM Qwen2.5-1.5B-Instruct - 配置: prefill_model_path={config.get('prefill_model_path')}")
