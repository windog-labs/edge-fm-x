#!/usr/bin/env python3
"""
Qwen2.5-VL-3B-Instruct 示例（EdgeFM 仅跑 LLM，图像需预计算 embedding 后通过 Request 注入）。
...
def main():
    ...
    print(f"EdgeFM Qwen2.5-VL 示例 - prefill_model_path={config.get('prefill_model_path')}")

    engine = edge_fm.EdgeFM(str(config_path))
    # 纯文本示例 token（与 Qwen2.5 格式一致）
